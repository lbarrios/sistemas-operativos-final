...
{
	Ext2FSBlockGroupDescriptor *bg = block_group(blockgroup_for_inode(inode_number));
	unsigned int inode_index = blockgroup_inode_index(inode_number);
	std::cerr << "load_inode->inode_index" << inode_index << std::endl;
	unsigned int inode_table = bg->inode_table;
	unsigned int block_size = 1024 << _superblock->log_block_size;
	
	unsigned int inodes_per_block = block_size/_superblock->inode_size;
	std::cerr << "inodes_per_block: " << inodes_per_block << std::endl;
	unsigned int block_number = inode_index/inodes_per_block;
	std::cerr << "load_inode->block_number: " << block_number << std::endl;
	unsigned int block_offset = inode_index%inodes_per_block;
	std::cerr << "load_inode->block_offset: " << block_offset << std::endl;

	Ext2FSInode inode_table_buffer[inodes_per_block];
	unsigned int block_address = inode_table+(block_number);

	read_block(block_address, (unsigned char*)inode_table_buffer);
	Ext2FSInode* inode = new Ext2FSInode;
	*inode = inode_table_buffer[block_offset];
	std::cerr << "INODE ORIGINAL: "<< inode_table_buffer[block_offset] << std::endl;
	std::cerr << "INODE COPIA: "<< *inode << std::endl;
...
{
	std::cerr << "block_number: " << block_number << std::endl;
	if(block_number<12)
	{
	...
	{
		std::cerr << "MAS DE 12" << std::endl;
		
		unsigned int block_size = 1024 << _superblock->log_block_size;
		unsigned int pointers_per_block = block_size / (sizeof(unsigned int));
		std::cerr << "pointers_per_block: " << pointers_per_block << std::endl;
		if(block_number-12 < pointers_per_block)
		{
			// Single indirect block
			unsigned int single_indirect_block = inode->block[12];
			std::cerr << "single_indirect_block " << single_indirect_block << std::endl;
			unsigned int *buffer = new unsigned int [pointers_per_block];
			std::cerr << "sizeof(buffer): " << sizeof(buffer) << std::endl;
			/*
			for( int j=0; j<256; j++){
						std::cerr << "buffer: " << buffer[j] << std::endl;}
						*/
			read_block(single_indirect_block, (unsigned char *) buffer);
			std::cerr << "devolviendo buffer["<< block_number-12 << "] " << buffer[block_number-12] << std::endl;
			unsigned int result = buffer[block_number-12];
			delete buffer;
			return result;
		}
		else
		{
			std::cerr << "MAS DE 1aaaaaaaaaaaaaa" << std::endl;
			if(block_number-(12+pointers_per_block)<pointers_per_block*pointers_per_block)
			{
				// Double indirect block
				unsigned int block_offset = block_number-(12+pointers_per_block);
				// get the first block
				unsigned int first_indirect_block = inode->block[13];
				unsigned int first_buffer[pointers_per_block];
				read_block(first_indirect_block, (unsigned char*)first_buffer);
				// get the second block
				unsigned int second_indirect_block_number = (block_offset)/pointers_per_block;
				unsigned int second_indirect_block = first_buffer[second_indirect_block_number];
				unsigned int second_buffer[pointers_per_block];
				read_block(second_indirect_block, (unsigned char*)second_buffer);
				// return the second buffer
				return second_buffer[block_offset%pointers_per_block];
			}
			else
			{
...
//#define DIR_ENTRY_SIZE_FIXED (sizeof(unsigned char)+sizeof(unsigned char)+sizeof(unsigned short)+sizeof(unsigned int))
struct Ext2FSInode * Ext2FS::get_file_inode_from_dir_inode(struct Ext2FSInode * from, const char * filename)
{
	...
	unsigned int dir_entry_read = 0;
	unsigned int dir_entry_total_read = 0;
	unsigned int current_block = 0;
	while(dir_entry_total_read < from->size)
	{
		unsigned int block = get_block_address(from, current_block);
		std::cerr << "block addr 1:" << block << std::endl;
		unsigned int block2 = get_block_address(from, current_block+1);
		std::cerr << "block addr 2:" << block2 << std::endl;
		unsigned int block_size = 1024 << _superblock->log_block_size;
		unsigned char buffer[block_size*2];
		read_block(block, buffer);
		if(block2 != 0)
			read_block(block2, buffer+block_size);
		while(dir_entry_read < block_size && dir_entry_total_read < from->size)
		{

			Ext2FSDirEntry *dir_entry = (Ext2FSDirEntry*) &(buffer[dir_entry_read]);
			std::cerr << "HOLA:" << dir_entry_total_read << std::endl;
			unsigned int record_len = dir_entry->record_length;
			std::cerr << "\trecord_len:" << record_len << std::endl;

			/*
			if(dir_entry_read+record_len < block_size)
			{
			*/
			unsigned int name_len = dir_entry->name_length;
			std::cerr << "\tname_len:" << name_len << std::endl;

			char dir_filename[name_len+1];

			for(int i=0; i<name_len; i++){
				 dir_filename[i] = buffer[dir_entry_read+(8)+i]; /// record_len-name_len no anda!!!!!!!!!!!!!
			}
			dir_filename[name_len]='\0';
			
			std::cerr << "filename:" << dir_filename << std::endl;
			if(strcmp(dir_filename, filename)==0)
			{
				std::cerr << "LO ENCONTRE!!!!!!!!!!!" << dir_entry->inode << std::endl;
				// Son iguales, encontré el archivo
				Ext2FSInode* inode = load_inode(dir_entry->inode);
				std::cerr << "Voy a retornar: " << *inode << std::endl;
				return inode;
			}
			dir_entry_read += record_len;
			dir_entry_total_read += record_len;
			/*
			}
			*/
		}
		current_block++;
		dir_entry_read -= block_size;
	}
	return from;
}
...
int Ext2FS::read(fd_t filedesc, unsigned char * buffer, int size)
{
	...
	std::cerr << "realsize: " << realsize << std::endl;
	if(realsize > 0)
	{
